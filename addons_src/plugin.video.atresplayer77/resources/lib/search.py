...
import base64
import re
from .common import get_url, fix_img, _handle
...
def do_search(query=None):
    ...
    s = get_session()
    try:
        params = {"entityType": "ATPFormat", "size": 30, "page": 0}
        ...
        for item in results:
            ...
            if href:
                ...
                is_playable = type_ in ['VIDEO', 'EPISODE', 'MOVIE', 'LIVE']
                url_item = get_url(action='play', href=href) if is_playable else get_url(action='open_item', href=href)
                if is_playable: li.setProperty('IsPlayable', 'true')
                xbmcplugin.addDirectoryItem(_handle, url_item, li, not is_playable)
    except Exception as e: xbmcgui.Dialog().notification("Error Búsqueda", str(e))
    xbmcplugin.endOfDirectory(_handle)

def _p3_b64encode(value):
    if not isinstance(value, bytes): value = str(value).encode()
    value = base64.b64encode(value); length = len(value); part = int(length / 4)
    value = re.sub(b'=', b'', value); encoded = value[:part][::-1] + value[part:][::-1]
